...
#include "H:\SFML-2.3.2-windows-vc12-32-bit\SFML-2.3.2\include\SFML\Graphics.hpp"
#include "Node.h"
#include <ctime>
#include <math.h>
#include <vector>
...
float const GalaticCenterMass = 100;        //The mass of the very large object simulating a black hole at the center of a galixy;
...
unsigned int const NumParticles = 1500;        //Number of particles in simulation, currently 2^15                                
...
float MouseX = 0;
float MouseY = 0;
const float _GRAV_CONST = 0.1; //lower is slower but with better accuracy
std::vector<Body*> Bodies;                      //Container of all Bodies in simulation
Node GlobalNode;
bool IsPaused = false;                          //Contains the state of whether the simulation is paused or not
...
inline void CalculateForce(Body* bi, Body* bj, float pSoftener)  
{
